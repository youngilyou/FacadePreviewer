...
static int rtmp_command_onstatus(struct rtmp_t* rtmp, double transaction, const uint8_t* data, uint32_t bytes)
{
	...
	{
		// [YYIL] 2026-08-12 local patch (see thirdparty/librtmp/README.md "Local patches"):
		// was `assert(0)` -- crashed the publisher on any onStatus code this switch doesn't
		// enumerate, including our own RtmpVideoBridge's real deleteStream reply
		// ("NetStream.DeleteStream.Suceess", rtmp-server.c's rtmp_server_ondelete_stream,
		// upstream's own typo/quirk). An unrecognized status code from a real server is data,
		// not a programmer error -- log and continue instead of aborting the process.
		printf("%s: unrecognized onStatus, level: %s, code: %s, description: %s\n", __FUNCTION__, result.level, result.code, result.description);
		return 0;
	}
