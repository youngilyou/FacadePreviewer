...
## 2026-09-08: COLMAP 폴백 크래시 3연타 수정 + 1차 매칭(호모그래피 체인) 유령 현상 근본 원인 규명

`TestApt/TestBuilding/FRONT`(168장, 아파트 한 면) 실사용 중 스캔이 `NEEDS_MANUAL_REVIEW` →
COLMAP 폴백까지는 갔지만 최종 벽면 이미지가 안 나오는 문제를 발견해서 파고들다가, 실제로는
**서로 다른 원인의 버그 4개가 겹쳐 있었음**. 매번 "고쳤다"고 말하기 전에 재현/재검증했고, 그
과정에서 두 번은 실제로 부족했던 걸 사용자가 직접 지적해서 다시 파야 했음 — 아래 순서 그대로 기록.

### 버그 1 — `facade_plane_from_reconstruction`: 배경(산/지형) 점이 벽면 평면 크기를 왜곡

COLMAP이 만든 118,509개 3D 점 중 12.5%(14,771개)가 중심에서 100m 이상(최대 1.2km) 떨어진
배경(산) 점이었는데, 이 함수가 **이상치 제거 없이** 전체 점의 min/max로 벽면 크기를 계산해서
987m×1148m라는 말도 안 되는 캔버스가 나왔고, 1m당 100px로 렌더링하려다 11.3GB 할당 실패로
크래시(`cv2.warpPerspective` 내부, `pipeline/runner.py`의 넓은 except가 조용히 삼켜서 겉으로는
그냥 "COLMAP 폴백 결과 없음"으로만 보임). **수정**: `rectification.py`에 2단계 이상치 제거 추가
— (1) `_near_camera_track_mask`: 각 점의 최근접 카메라 거리가 이 캡처 전체의 중앙값×4(최대 150m
cap)를 넘으면 제외 (드론은 찍는 대상에 가까이 붙어서 날지, 배경 산은 훨씬 멀다는 사실을 이용,
평면 자체의 coherence보다 훨씬 강한 신호), (2) `_ransac_plane_inlier_mask`: 남은 점에 RANSAC
평면 피팅(5m 이내만 inlier). **1단계만으론 부족했음**: 카메라-거리 필터만으로 87%가 통과했고
RANSAC이 그중 99.7%를 "평면 위"로 인정해버려서(사실상 전체를 다 씀), 벽면 크기는 우연히
그럴듯해 보였지만(39.5m×41.8m) 실제로는 여전히 오염된 상태 — 뒤에 버그 3에서 재발.

### 버그 2 — `rectify_images`: 이미지 165장을 전부 "캔버스 전체 크기"로 저장 → 24GB 메모리

`rectify_images`가 원래 "캔버스가 벽면 크기로 고정돼 있으니 문제없다"는 설계 의도로 각 이미지를
자기가 실제로 덮는 작은 영역이 아니라 **캔버스 전체 크기**로 워프해서 저장하고 있었음 — 드론 사진
한 장은 벽면의 일부만 찍는데 165장 전부를 전체 캔버스 크기 사본으로 들고 있으니 메모리가
23~24GB까지 치솟아 또 한 번 크래시. **수정**: `warp.py`의 체인 방식과 동일하게 각 이미지를
투영된 실제 영역(로컬 ROI)+offset(`corner`)만 저장하도록 변경.

### 버그 3 — corner 반올림 오프바이원 + `np.linalg.svd`의 `full_matrices=True` 기본값

버그 2를 고치고 재실행하니 이번엔 `cv2.detail` 블렌더가 "Unknown C++ exception"으로 크래시 —
원인은 ROI의 corner를 `round()`, 크기를 별개로 `ceil()`로 계산해서 `corner+size`가 캔버스보다
1px 넘어갈 수 있었던 것(블렌더가 이걸 허용 안 함). `floor()`로 corner를 정하고 그 지점 기준으로
size를 캔버스에 맞춰 다시 clamp하도록 수정.

그 다음 재실행에서 **버그 1의 잔재가 실제로 재발**함을 발견 — `facade_plane_from_reconstruction`의
`np.linalg.svd(points - centroid)`가 point 개수(N)가 10만 개대일 때 기본값 `full_matrices=True`
때문에 필요도 없는 (N,N) `U` 행렬(74.6GB!)을 할당하려다 실패. `full_matrices=False`로 수정(이
함수는 애초에 `Vt`만 쓰고 `U`는 버림 — 수학적으로 결과 동일, 메모리만 차이). **이 수정 하나로
캔버스가 3950×4182 → 5889×4133로, 사진 한 장당 투영 크기가 중앙값 13,000×9,700px(캔버스보다
훨씬 큼, 대부분 화면 밖으로 잘림)에서 2,900~3,300×2,300~2,600px(캔버스와 비슷한 비율)로 정상화됨.**
카메라 165대 전부 벽면에 거의 수직(cos≈1.000)으로 일정 거리(~21m)에서 찍은, 사실은 매우 깨끗한
데이터였다는 것도 이때 확인 — "기울어진 각도 때문"이라는 가설은 틀렸고 순수 메모리 버그였음.

**결과 (버그 1~3 수정 후 COLMAP 경로)**: coverage_ratio 28.3%(137/165장 기여) → **96.3%(165/165장
전부 기여)**. 사용자가 실제 원본 사진(`DJI_0100.JPG`)과 결과물을 나란히 비교해서(분홍 지붕 띠,
옥상 원형 환기구, 검은 지붕 구조물 등 건물 고유 디테일 일치) 진짜 원본 픽셀로 만들어진 게 맞다고
확인.

### 버그 4 (별도 계통) — 1차 호모그래피 체인의 "유령처럼 겹침" 근본 원인

COLMAP은 고쳤지만 1차(단순 페어 매칭 체인) 결과는 여전히 참혹함(`global_drift_score_px=478.5`,
허용치 10의 48배, `max_drift_score_px=5725.7`, 허용치 20의 286배) — 사용자가 "먼저 스티칭부터
잡으라"고 지시해서 근본 원인 추적:

**4-1) `compute_global_homographies`가 여분의 엣지를 버림**: `graph.py`가 기준 이미지로부터
BFS 스패닝 트리 경로 하나만 골라서 호모그래피를 이어붙이는데, 실제 그래프엔 그 트리에 필요한
164개 외에 **1,779개의 여분(cycle) 엣지**가 더 있었음 — `compute_drift_score`가 이걸 "측정"에만
쓰고 실제 보정엔 전혀 안 씀. 즉 체인 어딘가 나쁜 매칭 하나가 있으면 그 뒤 전부가 틀어지는데,
바로잡을 수 있는 정보(다른 경로들)를 버리고 있었음. **수정**: `refine_global_homographies`
추가 — 트리 뿐 아니라 전체 엣지를 동시에 써서 `scipy.optimize.least_squares`로 전역 최소제곱
보정(bundle adjustment). 실제 165장 데이터로 검증하려면 재매칭에 1시간 45분이 걸려서, 먼저
**합성 데이터(정답을 아는 10개 이미지 그래프)로 수학적 검증**부터 함:
  - 일반 L2 손실: 오히려 악화(3.5px→6.0px) — 완전히 틀린 매칭 하나(반복 패턴 오매칭을 흉내낸
    outlier edge)가 전체를 잘못된 방향으로 끌고 감
  - Huber 손실(이상치에 강건)로 전환 + `f_scale=2.0`: 개선되긴 했지만 미미(3.5→3.6px) —
    원인은 8개 호모그래피 파라미터의 단위가 서로 완전히 다름(perspective항 ~1e-5, 회전항 ~1,
    이동항 ~10-100px)이라 `scipy.optimize.least_squares`의 trust-region 스텝이 제대로 안 잡힘
  - `x_scale='jac'`(자동 파라미터 스케일링) 추가 후: 5번의 랜덤 시드 테스트 전부에서 일관되게
    개선(정답 대비 오차 최대 73% 감소) — 이 조합(huber, f_scale=2.0, x_scale='jac')으로 확정.
    **주의**: 아직 실제 165장 데이터로는 검증 못 함 — 재매칭(1차 페어 매칭)을 다시 돌려야
    확인 가능, 합성 테스트로 수학적 타당성만 확인된 상태.

**4-2) `select_pairs`가 고도(altitude)를 완전히 무시** (사용자가 "1차 매칭이 왜 이렇게 오래
걸리냐"고 물어보다가 발견): 168장 매칭에 6,217페어가 나와서 LoFTR 1페어당 약 1초씩, 총
1시간 45분이 걸렸음. 원인 추적 결과 `pair_selector.py`의 공간 근접 페어 선택(`_local_enu_xy`)이
**위도/경도만 쓰고 고도를 아예 안 씀** — 그런데 이 건물 촬영은 지그재그로 위아래(고도 35.5m
범위, 67.7~103.3m)를 오가는 패턴이라 수평 이동은 작고 고도 변화가 큼. 실측 확인: "수평 거리
1m 이내"로 잡힌 페어 2,241개 중 **82%(1,833개)가 실제로는 고도 차이 5m 이상**(=다른 층, 전혀
안 겹침)인데도 매칭 후보로 올라가고 있었음 — 반복되는 창문/발코니 패턴이 있는 벽면에서 이건
바로 "다른 층의 똑같이 생긴 창문끼리 자신 있게 잘못 매칭"으로 직결됨(1차 체인이 유령처럼
겹치던 진짜 근본 원인일 가능성이 높음, 버그 4-1과는 별개 계통).

**수정**: `_gps_distance_3d`(수평 haversine + 고도차를 유클리드로 합산) + `_local_enu_xyz`(KD-tree도
3D)로 전환. 실측: 3D 기준 진짜 이웃 사진 간격은 90%가 2.5m 이내인데 반해, 기존 20m 반경은
차원이 다르게 넓었음. **반경을 고정값으로 박지 않고 자동 계산**하도록 `_auto_calibrate_radius`
추가 — 이 촬영 자체의 최근접 이웃 3D 거리 90퍼센타일 × 3배(경험적으로 검증: 진짜 이웃은 2.5m대,
다른 층은 5m+부터 시작하므로 3배면 그 간격 안에 안전하게 들어감), `max_gps_distance_m`(8m)은
이제 폴백/상한 캡 역할만 함. **사용자 지시로 명확히 기록**: 지금은 일반 GPS 기준으로 설계했고,
추후 RTK GPS(cm급 정밀도)로 업그레이드 예정 — 자동 계산 방식이라 RTK로 바뀌어도 실측 이웃 거리가
더 촘촘해지면 반경도 자동으로 더 타이트해짐, 코드/설정 변경 불필요. 실측(GT 노이즈 없는 시뮬레이션
GPS라 실제 드론보다 훨씬 깨끗하다는 점은 별도로 사용자에게 고지함): 168장 기준 페어 수
6,217 → **888개**(temporal_neighbor 612 + gps_proximity 276), 약 7배 감소 — 매칭 시간도
1시간45분 → 약 15분으로 단축 예상.

**COLMAP 쪽도 같은 문제 있음 (아직 미수정)**: `pycolmap.match_spatial`이 GPS 기반 매칭을
지원하지만 기본값이 `ignore_z=True`(고도 무시) — 똑같은 함정이 COLMAP 쪽에도 그대로 있다는 걸
확인함. 게다가 지금 `run_colmap()`은 COLMAP DB에 GPS prior 자체를 안 쓰고 있어서(`match_exhaustive`
사용), `match_spatial`로 바꾸려면 DB에 위치 prior를 쓰는 배선 작업이 추가로 필요함 — 범위가
커서 이번엔 안 하고 별도 작업으로 남김.

**실제 168장 재검증 결과 (버그 4-1+4-2 적용 후)**: `global_drift_score_px` 478.5 → **10.09**
(48배 개선), `max_drift_score_px` 5725.7 → **218.0**(26배 개선) — bundle adjustment가 실제
데이터에서도 확실히 작동함을 확인. 하지만 **부작용 발견**: `disconnected_components` 5 → 20,
unreachable 이미지 15장 → 137장으로 오히려 악화 — 페어를 6,217→888개로 너무 줄이면서 그래프
연결성 자체가 나빠짐(기준 이미지 `DJI_0109` 주변 ~28장만 연결되고 `DJI_0004~0088`,
`DJI_0117~0171` 두 덩어리가 통째로 끊김). 사용자에게 "COLMAP이 최종이니 1차 스티칭 커버리지는
안 중요할 수 있다"고 제안했으나 **명확히 거부당함 — "둘 다 중요, 스티칭도 COLMAP도"**.

### 버그 5 — temporal_neighbor(촬영 순서상 이웃)에도 GPS 거리 체크가 걸려서 지그재그 전환 지점마다 체인이 끊김

위 연결성 악화의 진짜 원인: `select_pairs`의 `add()` 함수가 **모든** 후보 쌍(공간 근접이든
촬영순서 이웃이든 상관없이)에 똑같이 GPS 3D 거리 체크를 걸고 있었음. `temporal_neighbor_count=4`로
"촬영 순서상 바로 다음 4장은 반드시 연결"하려는 의도였는데, 지그재그 패스가 바뀌는 지점(한
세로 패스 끝 → 다음 세로 패스 시작)에서 실제 이동 거리가 자동 계산 반경보다 커지면 이 "반드시
연결돼야 할" 링크마저 거부되어, 매 패스 전환마다 체인이 끊기고 20개의 섬으로 쪼개졌음.

**수정**: `add()`에 `require_distance_check` 파라미터 추가, temporal_neighbor 페어는 GPS 거리
체크를 건너뛰도록(view-angle 체크는 유지 — 이건 물리적 거리와 다른, "실제로 회전해서 안 겹칠
수 있다"는 별개의 신호라 남겨둠) 변경. 근거: 계획된 패턴으로 촬영하는 사진은 촬영 순서상 이웃이면
설계상 겹치게 돼 있으므로, 촬영 순서 자체를 신뢰하는 게 맞고, "실제로 안 겹치는 촬영 순서상
이웃"을 GPS로 거르려는 시도가 오히려 체인을 끊는 역효과를 냄.

**검증(재매칭 전, 후보 그래프 구조만)**: `networkx`로 확인 — temporal_neighbor 엣지만으로도
168장 전체가 **1개의 연결 요소**(끊김 없음)를 이룸을 확인. 전체 페어 수는 888→938개(소폭 증가,
거리 때문에 빠졌던 일부 temporal_neighbor 페어가 복원됨)로 매칭 속도에는 거의 영향 없음.
**아직 실제 LoFTR 재매칭까지는 안 돌려봄** — 후보 그래프가 연결돼 있다는 것과 실제 매칭이
성공해서 최종 그래프도 연결된다는 것은 다른 얘기라, 다음 재실행에서 실측 확인 필요.

**아직 안 한 것**:
- 버그 5까지 반영한 상태로 168장 처음부터 재실행해서 최종 검증 필요(진행 중이던 이전 실행은
  버그 5 수정 전 코드로 돈 것이라 COLMAP 결과만 참고용, 1차 스티칭 연결성 검증은 무효).
- COLMAP 자체의 `match_spatial`+GPS prior 배선(위 참고, 아직 안 함).
- `pipeline.yaml` 헤더 주석이 "images arrive pre-downscaled to 640x640"라고 돼 있는데 실제
  `FRONT/*.JPG`는 5280x3956 원본 — 문서와 실제가 어긋나 있음, 오늘 건드린 범위 밖이라 발견만
  기록.

### 실제 촬영 패턴 확인 (사용자 진술) — 버그 5의 진단이 정확했음을 뒷받침

사용자 설명: "이미지 촬영은 아래에서 탑까지 촬영, 좌/우 이동시 촬영 중단, 탑에서 지면으로
내려오면 촬영, 좌/우 이동 촬영 중단 이렇게 촬영 하였습니다." 즉 세로 패스(위/아래) 동안만
연속 촬영하고, 패스 사이 좌우 이동 중에는 셔터를 아예 끔(그 구간 사진 자체가 없음) — 위/아래
번갈아 촬영하는 지그재그(boustrophedon) 패턴.

이건 버그 5에서 이미 관측했던 현상(20개로 쪼개진 섬, 끊긴 지점이 촬영 순서상 패스 경계와
일치)의 원인을 정확히 설명함: 각 세로 패스 끝(예: 맨 위)과 다음 패스 시작(다시 맨 위, 방향이
번갈아 바뀌므로)은 실제로는 "같은 높이, 옆 칸"이라 물리적으로 겹칠 가능성이 높지만, **자동
보정 반경은 패스 내부(세로) 간격 기준으로 계산돼 있어서 패스 사이 가로 이동 거리가 그보다
크면 거리 컷에 걸림** — 정확히 버그 5가 고친 그 지점. `temporal_neighbor_count=4`가 촬영
순서(image_id) 기준이므로, 좌우 이동 중 촬영이 없다는 것 자체가 "패스 경계의 두 사진은 항상
image_id가 바로 연속"이라는 뜻이 되어, 버그 5의 수정(temporal_neighbor는 거리 체크 생략)이
이 정확한 경계 쌍을 이미 포함하고 있음 — **추가 코드 변경 불필요**, 기존 수정이 이 패턴에
그대로 들어맞음.

남은 리스크(코드로 처리하지 않고 자연히 걸러지는 부분): 만약 실제 가로 이동 폭이 사진
한 장의 시야각보다 넓어서 패스 경계 두 장이 진짜로 안 겹친다면, 거리 체크는 안 걸리지만
LoFTR 매칭/geometry 게이트(`min_matches`/`min_inliers`)가 정상적으로 실패해서 그 엣지 하나만
그래프에서 빠짐 — 억지로 잘못 연결하지 않음. 그 경우 근처 `gps_proximity` 페어가 대신 다리
역할을 해줄 수 있는지는 실제 재매칭으로만 확인 가능.

### 168장 COLMAP 경로 실측 재검증 — 96%대 커버리지, 연결 요소 1개 (verified, 사진으로 확인)

버그 5 적용 전 코드로 돌던 백그라운드 작업(`bkfi5jz8a`, pair_count=888 = 버그 5 이전 값)이
완주함 — COLMAP은 자체 매칭을 따로 돌리므로 `pair_selector.py` 변경과 무관, 이 COLMAP 결과는
그대로 유효한 참고값. 다만 파이프라인 마지막 단계에서 `FRONT_analysis_colmap.tif` 저장 중
`OSError: [Errno 22] Invalid argument`로 실패해 출력이 갱신되지 않음(버그 4 검증 때 만들어둔
`finish_colmap_rectify.py`로 같은 재구성을 재사용해 재실행 → 재현 안 됨, 코드 자체의 결정론적
버그가 아니라 일회성 OS 레벨 파일 접근 충돌로 추정 — `src/common/imageio.py`의
`imwrite_unicode`에 짧은 재시도(최대 5회, 0.5초씩 백오프)를 추가해 방어).

재실행 결과(실측, `finish_colmap_rectify.py`로 동일 재구성에서 rectify_and_blend 재실행):
`image_count=161`(요청 161장 전원 등록), `coverage_ratio=0.9582`(95.8%), `disconnected_components=1`,
`unreachable_image_ids=[]`, 캔버스 6310x4149. PNG로 변환해 직접 확인 — **아파트 정면 전체가
바닥부터 옥상까지, 좌우 전체 폭에 걸쳐 잘림 없이 나옴**(사용자가 지적한 "정면 전체가 나와야
한다" 요구를 충족). 눈에 보이는 잔여 흠: (1) 배경(하늘/산) 영역이 조각조각 이어붙인 듯한
콜라주 패턴으로 남아 있음(건물 자체가 아니라 배경만의 문제, RANSAC 평면 근처로 살아남은
배경 포인트들의 잔여 워핑으로 추정), (2) 건물 폭 방향 각 세로 베이(bay) 경계에서 얇은
수직 경계선이 보임(실제 발코니 돌출부의 시차일 수도, 잔여 스티칭 이음선일 수도 — 미판별),
(3) 우측 하단에 작은 검은 삼각 공백. 벽면 텍스처 자체(창문/발코니/에어컨 실외기 배치)는
왜곡이나 유령 이미지 없이 선명함 — 이전에 사용자가 "거짓말" 이라 지적했던 뒤틀린 결과와는
질적으로 다름.

**아직 안 한 것**: 이 COLMAP 수치는 버그 5(temporal_neighbor 거리체크 제외) 반영 전 매칭
그래프 기준 — 버그 5 이후 pair_selector로 COLMAP도 다시 돌리면 등록률/커버리지가 더 오를
가능성 있음(현재 161/168 = 7장은 애초에 후보 페어 자체가 없어서 COLMAP도 등록 못 했을 수
있음, 미확인). 1차 스티칭(BFS+refine) 경로는 버그 5 이후 코드로 아직 처음부터 재실행 안 함 —
다음 전체 재실행에서 두 경로 모두 최종 검증 필요.

## 2026-09-08/09: 계층적(형/열) 스티칭 추가 — 사용자 제안, 구현 중 진짜 버그 2개 발견/수정

사용자가 정확한 촬영 패턴을 알려줌: 아래→위 연속촬영, 좌/우 이동 중엔 촬영 중단(사진 자체가
없음), 위→아래 연속촬영, 다시 좌/우 이동 중 촬영 중단 — 세로 지그재그(boustrophedon) 패턴.
GPS 수평거리 점프(3m 기준, `src/stitching/passes.py::assign_pass_ids`)로 실제 168장을 나눠보니
정확히 6개 패스(28~29장씩)로 갈라졌고 경계가 실측 좌우이동 지점과 정확히 일치 — 패턴 이해가
정확했음을 확인.

이어서 사용자가 "1차 스티칭을 형(패스) 단위로 먼저 붙이고, 그 다음 열 단위로 다시 붙이면
어떤가" 제안. 타당한 이유: 지금 방식은 168장·938쌍을 하나의 평평한 그래프로 취급해서, 어디든
약한 엣지 하나가 유일한 경로 위에 있으면 그 아래 전체가 오염됨. 계층 구조는 이걸 (1) 각
패스를 그 패스 내부 엣지로만 독립적으로 붙이고 (2) 패스 경계를 넘는 엣지들로 패스끼리를
붙이는 훨씬 작은 2단계 그래프로 바꿈. `src/stitching/graph.py::compute_hierarchical_homographies`
로 구현, `pipeline.yaml`의 `stitch.hierarchical_passes: true`로 기본 활성화.

**버그 A (구현 중 발견, 즉시 수정)**: 처음 버전은 "패스"를 그대로 합성 단위로 썼는데, 패스
내부에서도 k=1 매칭 실패가 흔하다는 걸 이미 알고 있었음에도(패스 경계뿐 아니라) 이 사실을
반영 안 해서, 패스 하나가 내부적으로 쪼개지면 `pick_reference`가 고른 조각이 아닌 나머지
조각은 그냥 버려짐 — 실측 결과 오히려 연결성이 나빠짐(coverage_ratio 0.93→0.72). **수정**:
합성 단위를 "패스"가 아니라 "각 패스 서브그래프의 실제 연결 요소(connected component)"로
바꿔서, 패스 내부에서 쪼개진 조각도 다른 패스의 조각들과 동등하게 다리를 놓을 기회를 갖도록
`compute_hierarchical_homographies`를 재작성. 합성 데이터(패스 내부에 끊김이 있고 장거리
엣지 하나로만 우회 가능한 7노드 그래프)로 먼저 검증 후 재적용.

**버그 B (더 근본적, 실제 매칭 결과 캐시로 통제 비교하다 발견)**: RANSAC이 매 실행마다
비결정적(시드 고정 안 됨)이라 재실행할 때마다 어떤 애매한 엣지가 통과/실패하는지 흔들려서,
"버전 전/후" 비교를 하려면 **같은 매칭 결과를 재사용**해야 함(`match_and_cache.py`로 168장
938쌍 매칭을 한 번만 돌려 pickle로 캐시 — 이후 알고리즘 비교는 이 캐시로 몇 초 만에 재현
가능). 이 캐시로 flat vs 계층 방식을 통제 비교하니: **연결성(46 vs 33 reachable)이 서로
다름** — 그런데 두 방식 다 같은 매칭 그래프를 쓰므로 이론상 계층 방식이 flat보다 나쁠 수 없음.
원인 추적 결과 `pick_reference`(연결도 최대 노드 선택)가 **가장 큰 진짜 연결 성분이 아니라
우연히 다른(더 작은) 섬**을 고를 수 있다는 걸 발견 — flat은 DJI_0109가 속한 섬을, 계층
방식은 DJI_0060이 속한 섬(서로 완전히 겹치지 않는 다른 섬)을 골라서 생긴 차이였음, 스티칭
알고리즘 자체의 결함이 아니었음. **수정 1**: `pick_reference`가 항상 가장 큰 연결 성분
안에서만 고르도록 변경. 재검증하니 여전히 결과가 안 바뀜(!) — **수정 2**: 계층 방식의
2단계(패스 그래프)에서는 노드가 "패스"라서, "가장 큰 성분"을 패스 개수로 재면 안 되고
**그 패스들이 대표하는 실제 이미지 수 합**으로 재야 함(작은 조각 여러 개짜리 성분이 큰 조각
하나짜리 성분보다 "노드 수"는 많아도 실제 이미지는 더 적을 수 있음) — `pick_reference`에
`weights` 파라미터 추가, 패스 그래프 호출 시 `{pass_id: 이미지수}`로 가중치 부여. 이 두 수정
후 재검증: flat/계층 둘 다 정확히 같은 46/162장에 도달(동일 매칭 그래프이니 당연히 같아야
정상) — 이제서야 진짜 사과 대 사과 비교가 됨.

**최종 결론 (통제 비교 기준)**: 연결성 자체는 계층 구조로 못 늘림 — 애초에 매칭이 안 된
엣지는 어떤 합성 알고리즘도 만들어낼 수 없음(당연한 얘기지만 실측으로 재확인). 실제 라이브
전체 재실행 2회(각각 상황이 조금씩 다른 매칭 결과)에서는 드리프트가 극적으로 개선된 경우
(9.68→2.84px)와 거의 차이 없는 경우가 둘 다 있었음 — 연결된 성분이 클수록(체인이 길수록)
계층 구조가 드리프트를 줄이는 효과가 크고, 성분이 작으면 효과가 미미한 것으로 보임(완전히
확정된 결론은 아님, 표본 2회뿐). **`pick_reference`의 가중치 기반 최대 성분 선택 수정은
flat/계층 무관하게 항상 유효한 개선**이라 계속 유지.

**coverage_ratio가 오해를 부를 수 있음 발견**: 1차 스티칭에서 161장 중 48장만 배치됐는데도
`coverage_ratio=0.93`으로 높게 나온 사례 발견 — 원인은 캔버스 크기 자체가 "실제 배치된
이미지들의 범위"에 맞춰 자동으로 줄어들기 때문. 즉 coverage_ratio는 "그 좁아진 캔버스 안에서
얼마나 채워졌나"이지 "건물 전체가 나왔나"가 아님. 연결성이 나쁜 상황에서 이 지표만 보고
"잘 됐다"고 판단하면 안 됨 — `image_count`/`unreachable_image_ids` 크기를 항상 같이 봐야 함.

## 2026-09-09: GPS/짐벌 기반 포즈-일관성 검증 게이트 추가 (사용자 제안)

사용자가 반복 패턴 문제의 근본 해법으로 제안: "GPS로 두 사진의 예상 이동 방향을 계산해서,
RANSAC이 찾은 실제 호모그래피가 그 방향과 다르면 거부하자" — RANSAC은 자기 안에서만
일관되면 통과시키므로, 반복되는 층 패턴에서 "엉뚱한 층과 매칭됐지만 내부적으로는 일관된"
경우를 못 거른다는 게 근본 문제였음. GPS 고도는 매칭 결과와 완전히 독립된 신호라 이 맹점을
정확히 겨냥함.

**사전 검증 (구현 전 실제 데이터로 확인, 캐시된 168장 매칭 결과 재사용)**: 같은 고도차(dz)를
가진 k=1(바로 인접) OK 엣지들로 "고도 1m당 호모그래피가 암시하는 수직 픽셀 이동량" 관계를
확인 → 상관계수 0.80, 그런데 **같은 dz≈7.3~7.4m인데 어떤 쌍은 이동량 80px, 어떤 쌍은
1380px로 17배 차이**나는 이상 현상 발견. 짐벌 피치를 확인했으나 두 그룹 다 거의 수평(±0.2도)
이라 원인 아님 — 두 그룹의 고도대가 다름(작은 쪽 95~103m 옥상 근처, 큰 쪽 67~96m 중간층)이라
드론의 벽면 이격거리(standoff distance)가 구간마다 달랐을 가능성이 있으나 17배는 그걸로
설명하기엔 너무 큼 — **원인 미확정** (실제 매칭이 엉뚱한 층으로 미끄러진 것일 수도, 다른
이유일 수도 있음, 다음에 더 조사 필요).

**설계 결정**: 이 미확정 상태에서 절대 크기(스케일)까지 검증하면 스케일 추정을 잘못했을 때
멀쩡한 매칭을 거부하는 새 버그가 될 위험이 커서, **부호(방향)만 검증**하기로 함— "B가 A보다
높은 곳에서 찍혔으면 호모그래피도 반드시 같은 방향(같은 부호)을 가리켜야 한다"는 스케일과
무관하게 항상 참인 규칙. 이 규칙은 촬영 순서/비행 방향(올라가는 패스냐 내려가는 패스냐)과
무관 — 오직 두 사진의 실제 GPS 고도값 차이만 사용하므로, 어느 패스에서 왔든 동일하게 적용됨.

**구현**: `src/geometry/quality.py`에 `_gps_pose_consistent()`/`_implied_vertical_shift_px()`
추가, `apply_quality_gate()`가 선택적 `meta_a`/`meta_b`를 받아 REPROJECTION_ERROR 통과 후
마지막 게이트로 적용 (`GeometryFailureCode.POSE_INCONSISTENT` 신규). `src/pipeline/runner.py`의
호출부에 `by_id.get(pair.image_a/b)` 전달.

**임계값도 실측으로 튜닝** (처음엔 감으로 `min_dz_m=0.3, min_shift_px=5.0`로 짰다가 캐시
데이터에 돌려보니 499개 중 4개가 걸림 — 자세히 보니 걸린 것들의 ty가 전부 6~9px로, 이건 좋은
매칭들 사이에서도 호모그래피 추정 자체의 잔차 표준편차(~216px)에 완전히 묻히는 노이즈
수준이었음. **`min_dz_m=2.0, min_shift_px=100.0`으로 상향**(노이즈 바닥보다 확실히 위) 후
재검증하니 499개 전부 통과(오탐 0건) — 안전함을 확인. 단, 이 안전한(부호만 보는) 버전은
위에서 발견한 17배 미스터리 자체는 못 잡음(그 케이스들도 부호는 다 맞았고 크기만 튄
것이었음) — 그건 여전히 "스케일까지 보는 2단계"가 필요한, 아직 못 푼 문제로 남음.

**아직 안 한 것**:
- 17배 크기 미스터리의 진짜 원인(드론 이격거리 변화 vs 매칭 오류) 규명.
- 스케일까지 검증하는 2단계 게이트(원인 규명 전까지 보류).
- 새 게이트를 반영한 상태로 전체 파이프라인 재실행 검증(구현 직후라 아직 안 돌려봄 — 이
  게이트는 캐시 데이터에서 아무것도 안 걸렸으므로 이번 실제 168장 캡처에서는 효과가 거의
  없을 수 있음, 다른/실제 문제 있는 캡처에서 진가를 발휘할 안전장치 성격).

## 2026-09-09: 색 히스토그램 기반 근접성 검증 — 검토 후 기각 (실측)

사용자 제안: "각 사진의 컬러 히스토그램을 갖고 있으면, T-1/T/T+1이 가장 유사할 것이고, 열
방향으로 옆에 있는 것도 히스토그램 비교로 알 수 있지 않나 — 멀리 있으면 색이 비슷해도 값
차이가 날 것이다." 실제 이미지로 검증(HSV 히스토그램 상관계수):

| 그룹 | 평균 유사도 |
|---|---|
| k=1 바로 인접(진짜 가까움) | 0.994 |
| 정상(작은 이동량) 클러스터 | 0.920 |
| 의심(17배 큰 이동량) 클러스터 | 0.941 |
| 완전히 먼 쌍(다른 패스, 실제로 멂) | 0.729 |
| **같은 층처럼 보이는 다른 패스**(반복 패턴, 실제론 멂) | **0.900** |

"완전히 먼데 안 비슷한" 쌍은 잘 구분되지만(0.73 vs 0.92~0.99), 정작 잡아야 할 문제(반복
패턴 때문에 다른 층인데 비슷해 보임)는 색 히스토그램도 똑같이 속음 — 마지막 행이 진짜 가까운
쌍들(0.92~0.94)과 거의 구분 안 됨. 반복 패턴이 사람 눈과 RANSAC을 속이는 바로 그 이유(다른
층이 색까지 포함해서 비슷하게 생김)가 색 히스토그램도 무력화시킴. **기각.**

## 2026-09-09: 계층적(형/열) 스티칭 — 전체 원복 (사용자 결정)

2026-09-08/09 항목에서 만든 `compute_hierarchical_homographies`/`src/stitching/passes.py`/
`pipeline.yaml`의 `hierarchical_passes` 설정을 **사용자 지시로 전부 되돌림** — 이유: 통제
비교(같은 캐시된 매칭 결과로 flat vs 계층 방식 비교)에서 `pick_reference` 버그를 다 고치고
나니 두 방식이 **완전히 동일한 결과**(정확히 같은 46/162장, 소수점까지 같은 drift)를 냈고,
실제 라이브 재실행에서도 개선 효과가 있을 때(9.68→2.84px)와 거의 없을 때가 둘 다 있어서
일관된 이득이 증명되지 않음 — 반면 "2열 합치기" 실험에서 계산 비용(최소자승 문제 크기)이
크게 늘어나는 것도 확인됨(정확도 이득은 불확실한데 비용은 확실히 증가). 사용자 판단: "낳아진
것 없잖아요"(개선된 게 없지 않냐) — 복잡도만 추가하고 증명된 이득이 없으니 되돌리기로 함.

**되돌린 것**: `src/stitching/mosaic.py`의 `stitch_facade`(다시 항상 flat
`compute_global_homographies`+`refine_global_homographies`만 사용, `pass_ids` 파라미터
제거), `src/stitching/graph.py`의 `compute_hierarchical_homographies` 함수 삭제,
`src/stitching/passes.py` 파일 삭제, `src/pipeline/runner.py`의 `assign_pass_ids` 호출부
제거, `pipeline.yaml`의 `hierarchical_passes`/`pass_boundary_horizontal_m` 삭제.

**유지한 것** (계층적 스티칭과는 별개로 검증된 진짜 버그 수정이라 되돌리지 않음):
- `pick_reference`가 이제 항상 **가장 큰 연결 성분** 안에서만 기준점을 고름(이전엔 전체
  그래프에서 최대 연결도로 골라서 우연히 작은 섬을 고를 수 있었음) — flat 방식에도 그대로
  적용되는 실질적 개선.
- GPS/짐벌 부호-일관성 검증 게이트(`_gps_pose_consistent`, `POSE_INCONSISTENT`) — 계층적
  스티칭과 무관하게 독립적으로 유효, 오탐 0건 검증 완료.
- `generate_visual_mosaic: true`(멀티밴드 블렌딩 켜짐) — 다만 실측 결과 "잘라붙인 느낌"을
  "정렬 안 맞는 곳의 흐릿한 고스팅"으로 바꿀 뿐, 근본적인 정렬 오차 자체를 없애주진 않음을
  확인(둘 다 완벽하지 않은 서로 다른 트레이드오프).

**교훈**: 오늘 하루 종일 지켜온 "구현 전에 실측으로 검증" 원칙이 이번에도 핵심이었음 —
계층적 스티칭이 "논리적으로 더 나아 보인다"는 것과 "실제로 측정 가능한 개선을 준다"는 것은
다른 이야기였고, 통제 비교(RANSAC 비결정성을 제거한 동일 매칭 데이터 재사용)를 하지 않았다면
"라이브 재실행에서 드리프트가 좋아졌다"는 착시(실제로는 매칭 결과 자체가 그때그때 달라서
생긴 것일 수 있음)를 실제 알고리즘 개선으로 오인했을 수 있음.

## 2026-09-09: `colmap.mode: always` 실측 + 재촬영(150장, FRONT 폴더) 재분석

**`colmap.mode: always` 실측 결과** (구 캡처, 168장): 1차 LoFTR 매칭/스티칭을 완전히 건너뛰고
COLMAP에 전체 카탈로그를 바로 넘기는 `_run_colmap_only_pipeline` 경로 — 168/168 전부 등록
성공(기존 fallback 경로보다 등록률 좋음). 그런데 **시간 절약은 없음** — COLMAP 자신의
SIFT+증분 SfM이 LoFTR 사전 필터링 없이 전체 이미지를 다 처리해야 해서, 1차 스티칭에 쓰던
시간만큼 COLMAP 자체가 더 오래 걸림(총 소요시간은 fallback 모드와 비슷). `pipeline.yaml`에
`colmap.mode: always`로 유지 중인 이유는 시간 절약이 아니라 등록률/신뢰성(COLMAP의 전역 번들
조정이 반복 패턴에 더 강건) 때문.

**재촬영 데이터 재분석** (`D:\ClaudePr\UE_TemImg\TestApt\TestBuilding\FRONT`, DJI_0044~0193,
150장 연속, 2026-09-09 촬영, 고도 67.79~103.47m — **사용자 지시로 이 폴더만 사용, "FRONT -
복사본" 백업 폴더나 구 캡처 데이터는 참조 금지**): `colmap.mode: always`로 재실행 →
**150/150 전부 등록 성공**, `coverage_ratio=0.9681`(지금까지 최고 결과). 발코니 돌출
구조(bay window) 부위에서 국소적 정렬 오차 발견 — 원인 확인: COLMAP의
`facade_plane_from_reconstruction`이 건물 전체에 **평면 하나**만 피팅하는데, 실제 발코니는
평평한 벽면보다 돌출된 3D 굴곡이라 그 지점만 평면 근사가 안 맞는 것 — **버그가 아니라 평면
정류 방식 자체의 구조적 한계**(평평한 벽면 구간은 선명함). 이번 세션에서는 더 손대지 않음.

## 2026-09-09: 반복 패턴/격자 주기 검출 논문 5편 실사 검증 (신뢰 훼손 사건 이후)

Wolff et al. 2016(CVPR, median-tile 기법)에 대해 WebSearch 요약만 보고 세부 내용을
단정적으로 설명했다가, 사용자가 "당신을 믿을수 없어서요"라며 직접 반박(실제로는 PDF가
`D:\ClaudePr\UE_TemImg`에 이미 로컬로 있었음) — **이후로는 실제 PDF를 Read 도구로 전체
읽지 않고는 어떤 논문 내용도 단정하지 않기로 함**(추측/검색요약 기반 주장 전면 금지, 확인
못하면 "확인 못했다"고 솔직히 보고).

**정정된 내용 (Wolff et al. 2016)**: median-tile 기법은 층별 세부사항을 구분하는 데 쓰는
게 아니라 오히려 **평균으로 지워버려서(AVERAGE OUT)** 건물 단위(층 단위 아님) 식별을 돕는
용도 — 처음 추측과 정반대.

**Park, Brocklehurst, Collins, Liu (ACCV 2010)** "Translation-Symmetry-based Perceptual
Grouping" — 전체를 Read로 실사 확인. KLT+MSER+SURF 특징 취합 → mean-shift 클러스터링 →
**퍼스펙티브(단순 affine 아님) 변형 하에서** RANSAC으로 (t1,t2) 기저 벡터 추정 → NCC 기반
반복적 격자 완성. 자체 벤치마크 기준 precision~85%/recall~65%. 공개 코드 없음(검색으로
못 찾음).

**Pritts, Chum, Matas (CVPR 2014)** "Detection, Rectification and Segmentation of Coplanar
Repeated Patterns" — 격자(lattice) 가정이 필요 없는 더 일반적인 방법. MSER+LAF+SIFT,
무한선(line-at-infinity) 추정에 스케일-비율 제약, 반사/회전된 반복체로부터의 길이 제약으로
similarity 업그레이드, 방사왜곡(radial distortion)까지 포함한 비선형 결합 최적화. **저자
공식 구현체 존재 확인**(`gh api`로 GitHub `ivashmak/repeats`/`prittjam/repeats` 저장소의
BibTeX 인용/논문 PDF URL이 실제로 이 논문과 일치함을 검증) — 단 MATLAB, Linux 전용 mex
바이너리, 문서 다수가 "TBD"로 미완성이라 실사용 장벽이 큼. **이 평가가 사용자의 "리눅스
사용 안함, 매틀랩 사용 안함, Python으로 재직접 구현" 결정으로 직결됨** (아래 항목).

**Wenzel, Drauschke, Förstner (2008)** "Detection of Repeated Structures in Facade Images"
— 한 이미지 안에서 SIFT를 자기 자신과 매칭(self-matching), 방향/스케일 가중치, 2D 이동량
히스토그램, MDL 기반 계층적 기저 탐색. **사전에 정류(rectify)된 정면 이미지가 필요** —
원본의 기울어진(oblique) 사진은 그대로 처리 못함. **저자 스스로 "이웃한 유사 구조에 매우
민감하다(very sensitive to similar structures in the neighborhood)"고 명시적으로 인정** —
바로 이 프로젝트가 겪고 있는 정확히 그 문제를 저자들도 못 풀었다고 자인한 것.

**Kushnir & Shimshoni (PAMI 2014)** — 저자 서버(`is-web.hevra.haifa.ac.il`)가 curl/WebFetch
양쪽으로 두 차례 시도 모두 연결 자체가 안 됨(timeout/ECONNREFUSED) — **못 읽음**, 추측 안 함.

**Zhao et al. 2012** "Per-Pixel Translational Symmetry Detection" — 검색으로 정확한 저자/
출처를 특정 못함 — **못 읽음**, 더 정확한 인용정보 필요.

## 2026-09-09: `facade_period.py` — 반복 패턴 논문을 직접 Python으로 재구현 (사용자의 명시적 도전 과제)

사용자 지시: "리눅스 사용 안함, 매틀랩 사용 안함. 알고리즘을 Python으로 재구현 하세요 1%의
가능성이 있으면 해봐야 지요." — Pritts 2014의 공식 구현체가 MATLAB/Linux 전용이라 실사용
불가하다는 평가 직후, 정면돌파 지시.

**설계**: 논문 전체(격자-불필요, 완전 퍼스펙티브, 방사왜곡 보정)를 그대로 포팅하지 않고,
이 프로젝트의 실측 특성(짐벌 피치 거의 0 — 이미 이전 세션에서 검증됨)에 맞춰 **세로 방향
반복주기 하나만** 구하는 것으로 의도적으로 범위를 좁힘 — Wenzel 2008(자기-매칭 SIFT,
방향/스케일 가중치)과 Park 2010(RANSAC 합의 투표) 아이디어의 작은 조합. 신규 파일
`src/geometry/facade_period.py`: `estimate_vertical_period()` — ROI 크롭 → SIFT 자기매칭
→ 방향/스케일 가중치 필터링 → 후보 주기별 하모닉(배수) 합의 스코어링 → 최고 스코어 주기
반환(`PeriodEstimate(period_px, confidence, ...)`).

**발견/수정한 실제 알고리즘 버그**: 초기 버전은 후보 주기 T가 관측된 이동량 dy를 **제한 없는**
정수배로 "설명"할 수 있게 허용(`multiples = np.clip(multiples, 1, None)`) — 작은 스퓨리어스
주기(40~84px)가 우연히 여러 개의 크고 무관한 실제 이동량을 배수로 나눠 떨어뜨려서 100%
신뢰도로 이기는 문제 발생. **탐지 방법**: 실제 층높이(2.51m, 이전 세션에서 검증된 값)와
초점거리(px)로 측정된 주기를 "카메라-벽 거리"로 역산해 물리적 타당성 체크 → 135~285m라는
비현실적 값 확인(드론 촬영이면 10~30m대여야 함) → `DJI_0149.JPG`에서 실제 매칭된 SIFT 점
쌍을 직접 선으로 그려서 시각 확인 → 그려진 선이 실제로는 ~280~300px(한 층 분량)인데
보고값은 40px — 스퓨리어스 값이었음을 확정. **수정**: 하모닉 배수 상한 `max_multiple = 3`
추가. 재검증 결과 주기가 424~480px로 클러스터링, 역산 거리가 서로 다른 6장에서 일관되게
23.8~26.9m로 수렴(교차검증 통과) — `D:\ClaudePr\UE_TemImg\TestApt\TestBuilding\FRONT` 폴더
이미지로만 검증(사용자 지시 준수).

## 2026-09-09: `FLOOR_COUNT_MISMATCH` 게이트 — 크기(magnitude)까지 보는 2단계 검증 구현

2026-09-08/09에 만든 `POSE_INCONSISTENT`(부호만 검증, 스케일 미확정 문제로 보류됐던 부분)의
"보류된 나머지 절반"을 `facade_period.py`로 메움 — 사용자 지시: "이제 이걸 실제 매칭 검증
게이트(호모그래피가 암시하는 이동량 vs 이 픽셀 주기의 정수배)로 연결해볼까요."

**구현**: `src/geometry/quality.py`에 `_floor_count_consistent()` 추가 — GPS 고도차 dz를
`real_floor_height_m=2.51`로 나눈 `expected_floors`와, 호모그래피의 암시 이동량(ty)을 **이
이미지 자신의** 측정된 주기(`period_a.period_px`)로 나눈 `observed_floors`를 비교, 0.75층
넘게 불일치하면 거부(`FLOOR_COUNT_MISMATCH` 신규 코드, `types.py`). `period_a.confidence <
0.6`이면 항상 통과(정보 부족 시 거부 안 함 — `_gps_pose_consistent`와 동일 원칙). `types.py`,
`apply_quality_gate()` 시그니처(`period_a` 신규 옵션 파라미터), `src/pipeline/runner.py`(모든
카탈로그 이미지에 대해 매칭 루프 시작 전 `estimate_vertical_period()`를 한 번씩 미리 계산해
`periods` dict로 캐싱 후 `apply_quality_gate(..., period_a=periods.get(pair.image_a))`로
전달) 순서로 배선 완료.

**중요 발견 — 배선은 됐지만 현재 운영 설정에서는 미작동**: `pipeline.yaml`이
`colmap.mode: always`이므로 `run_facade_poc`는 `_run_colmap_only_pipeline`로 바로 가고,
그 경로는 LoFTR 매칭/`estimate_homography`/`apply_quality_gate`를 **아예 호출하지 않음**
(COLMAP 자신의 SfM만 씀) — 즉 이번에 만든 `FLOOR_COUNT_MISMATCH` 체크는 `_run_facade_pipeline`
(LoFTR 우선 경로)에서만 실행되는데, 현재 기본 설정은 그 경로를 타지 않으므로 **지금 그대로는
실제 운영에서 한 번도 안 불림**. 코드 자체는 맞게 짰지만 "배선 완료 = 실사용 중"이 아님을
사용자에게 명시적으로 보고함(가정으로 넘어가지 않기 위해).

**실측 검증**: `D:\ClaudePr\UE_TemImg\TestApt\TestBuilding\FRONT` 150장 전체로 `select_pairs`
+ 실제 LoFTR 매칭 + `apply_quality_gate(period_a=...)`를 (config의 mode 분기와 무관하게)
직접 재현 실행해 `FLOOR_COUNT_MISMATCH`가 실제로 발동하는지/오탐은 없는지 검증.

**검증 도중 겪은 별개의 버그(제 테스트 스크립트 한정, 운영 코드 무관)**: 첫 검증 스크립트가
모든 코드를 모듈 최상위(가드 없이)에 작성 — `TimeoutLoFTRMatcher`가 Windows `spawn` 방식으로
워커 프로세스를 띄우는데, 워커가 응답 못 하고 60초 타임아웃될 때마다 `_kill_and_respawn()`이
새 워커를 스폰하고, Windows spawn은 새 워커 준비 단계에서 `__main__`으로 실행됐던 스크립트
전체를 다시 통째로 실행함(`if __name__ == "__main__":` 가드가 없으면 이 재실행이 무조건
전부 수행됨) — 재실행된 스크립트가 다시 카탈로그 빌드+150장 주기 계산(5분)을 처음부터 반복
하다가 자기 자신의 `TimeoutLoFTRMatcher()` 생성 시도에서 "부트스트래핑 중에는 새 프로세스를
못 띄운다"는 파이썬 자체 가드에 걸려 크래시 → 진짜 워커는 끝내 응답을 못해서 60초마다 이
사이클이 무한 반복 — 실제로 30분 이상 하나도 진행 안 되는 것처럼 보였던 원인이었음(단순
출력 버퍼링 문제로 처음엔 오판했었으나, 실제로는 이 좀비 프로세스 폭주가 CPU를 잠식해서
진짜 못 끝난 것). **수정**: 스크립트 전체를 `def main(): ...` + `if __name__ == "__main__":
main()`로 감싸기만 하면 해결(표준적인 멀티프로세싱 안전 패턴). **운영 코드 확인**: 실제 제품
진입점 `tools/stitch_engine/stitch_folder.py`는 애초부터 이 가드가 올바르게 돼 있음을 코드로
직접 확인 — 이번 버그는 임시 검증 스크립트에만 있었고 실제 파이프라인은 영향 없음.

**최종 결과** (가드 수정 후 GPU로 269초 만에 760쌍 전부 처리, 이전엔 버그 때문에 30분+
멈춰있었음): `OK` 385건, `LOW_INLIER` 375건(게이트와 무관한 기존 매칭 자체의 인라이어 부족),
**`FLOOR_COUNT_MISMATCH` 0건**. `POSE_INCONSISTENT`도 0건. 즉 **오탐(false positive)은
0건으로 확인**(385개의 정상 OK 판정 중 하나도 부당하게 거부되지 않음 — 안전성 확인 완료).
다만 **실제로 뭔가를 잡아낸 사례도 0건**이라, 이 캡처 자체에 층-오매칭이 없었거나(가능성
높음 — 같은 이유로 `POSE_INCONSISTENT`도 이전에 캐시 데이터에서 0건이었음), 주기 신뢰도
조건(`confidence>=0.6`)을 만족한 이미지가 150장 중 70장(47%)뿐이라 게이트가 판단할 정보
자체가 부족했을 가능성 둘 다 남아있음 — 이번 캡처에서 개선 효과가 증명된 건 아니고, 다른/
문제 있는 캡처를 위한 안전망 성격의 코드로 봐야 함(`POSE_INCONSISTENT`와 동일한 위치).

## 2026-09-09: CUDA COLMAP+pycolmap 소스 빌드 (GPU dense reconstruction) 완성 + 파이프라인 통합 시도 → 실측 결과 역효과로 기본 비활성화

**배경**: 이전 세션에서 발견한 실제 결함 2건(좌우 반전, 모서리 벽면 이중선) 중 반전은 수정,
모서리는 이미지별 유효영역 마스킹(Option B)으로 수정 완료. 남은 발코니 왜곡("blue blob",
평평한 평면 가정이 실제로는 벽면에서 돌출된 발코니를 잘못된 위치로 비틀어 투영하는 문제)은
Option C(모서리별 평면 추가)로 시도했으나 물리적으로 불가능한 결과(68-148m 크기 "평면")만
나와 포기. 사용자 지시: "GPU/CUDA 이거 마저 하고, 버그를 하나하나 수정해 가지요" → GPU
경로(COLMAP 자체 dense reconstruction, Option D) 우선 진행 확정. COLMAP의 dense 모듈
(`patch_match_stereo`)은 CUDA 전용이라 CPU 구현 자체가 없음(`pycolmap.PatchMatchOptions`가
`gpu_index`만 노출, CPU 모드 없음 — upstream 확인) — 공식 PyPI `pycolmap` wheel(4.1.1,
최신 4.2.0도 동일)은 CPU-only 빌드라 소스에서 CUDA 활성화해 직접 빌드해야 함.

**Windows에서 CUDA COLMAP 4.3.0.dev0 + pycolmap 4.3.0.dev0 소스 빌드 (수 시간 소요, 겪은
문제 다수)**: vcpkg manifest 모드(`x64-windows-static` triplet, MSVC 런타임 `/MT` 강제 —
COLMAP 자체가 `/MT`로 컴파일되므로 vcpkg 라이브러리도 맞춰야 `LNK2038` 안 남), MSVC 툴셋
2개(14.33/14.44) 공존으로 인한 `__std_search_1` 등 unresolved-external(`vcvarsall.bat`에
`-vcvars_ver=14.44` 명시 고정으로 해결), PoseLib(FetchContent로 받아오는 서드파티)가 MSVC의
템플릿 partial-ordering 한계로 C2672 컴파일 실패(GCC/Clang은 통과 — MSVC만의 문제) → 소스
직접 패치(`jacobian_accumulator.h`의 `add_jacobian`/`add_residual` 오버로드 3개를
`enable_if` 기반 단일 템플릿으로 통합), `/permissive-`는 이 문제를 못 고치고 오히려
PoissonRecon을 새로 깨뜨려서 제거, `/bigobj`(bundle_adjustment_ceres.cc 섹션 초과),
pycolmap 서브빌드(`python/` 디렉터리, scikit-build-core)는 자체 vcpkg manifest 인식을 위해
`vcpkg.json`/`vcpkg-configuration.json`을 `python/`에 복사(+overlay-ports 상대경로 수정
필요), CGAL/CURL(`download` feature) 등 추가 vcpkg feature 필요, scikit-build-core가 기본
Visual Studio 제너레이터를 쓰면서 사용자가 지정한 MSVC 툴셋 버전을 무시하는 문제는
`CMAKE_GENERATOR=Ninja` 환경변수로 강제 우회. **최종 검증**: `colmap.exe`
(`patch_match_stereo`/`stereo_fusion` 커맨드 확인)와 `pycolmap`(`has_cuda=True`,
`get_num_cuda_devices()=1`) 둘 다 정상 빌드. **실제 140장 FRONT 데이터셋 전체로
`patch_match_stereo` 크래시 없이 완주 검증**(8분 10초, 뷰당 ~3.4초, geom_consistency=False
smoke test) — 이전에 3장짜리 임시 워크스페이스로 테스트했을 때 났던 크래시는 CUDA 빌드
자체의 결함이 아니라 그 테스트 워크스페이스의 데이터 불완전성(이웃 관계 미비) 때문이었음을
최종 확인.

**다른 PC 설치용 원스톱 스크립트**: 사용자 지시("다른 컴퓨터에서 설치 될수 있어아 함")로
`D:\ClaudePr\CheckCrackV2\scripts\install_colmap_cuda_dense.bat` +
`colmap_cuda_patches\jacobian_accumulator.patched.h`(패치된 최종본 파일 자체를 통째로
배포 — 자동 find/replace 패처보다 훨씬 안전) 작성. 설치 경로 `C:\temp` 고정, MSVC
툴셋/vcvars_ver은 `vswhere.exe`로 자동 감지해 최고 버전 고정(이 머신처럼 툴셋이 2개 있어도
안전), `CMAKE_CUDA_ARCHITECTURES=native`(CMake 3.24+, 빌드하는 그 머신의 실제 GPU를
자동 감지 — 이 머신 전용 `89` 하드코딩 대신 이식성 확보), 각 단계 idempotent(이미 결과물이
있으면 스킵, 재실행 안전). Visual Studio 2022(C++ workload)/CUDA Toolkit/Git/CMake/Ninja/
Python은 스크립트가 사전 확인만 하고 설치는 안 함(GUI 설치 프로그램이라 자동화 범위 밖,
없으면 정확한 다운로드 링크와 함께 즉시 중단).

**파이프라인 통합 — 시도했으나 실측 결과 역효과, 기본 비활성화로 되돌림**:
`src/geometry/dense_depth.py` 신규 — `is_gpu_dense_available()`(CUDA 감지, CPU 머신에서는
전체 기능이 조용히 no-op), `run_dense_stereo()`(UTM-정렬된 reconstruction을 임시
sparse로 다시 write 후 undistort+patch_match_stereo — **정렬 전 원본을 쓰면 COLMAP
reconstruction 특유의 임의 스케일 때문에 depth 값이 실제 미터 단위와 안 맞는 함정이 있어서,
반드시 GPS로 스케일이 확정된 aligned reconstruction을 다시 write해서 그걸로 dense stereo를
돌려야 함 — 처음에 이 함정을 발견하고 고침), `apply_depth_correction()`(각 픽셀을 카메라
레이+depth로 3D 세계좌표 복원 후 평면 법선 방향 편차를 계산, `deviation_threshold_m`~
`max_deviation_m` 구간에 있는 "진짜 벽면에서 살짝 돌출된" 픽셀만 정면 스캐터 방식으로
캔버스에 다시 투영). `rectification.py`(`rectify_images`/`rectify_and_blend`에 배선),
`runner.py`(양쪽 호출부에 `dense_workspace_dir` 전달), `pipeline.yaml`(`dense:` 섹션 신규)
전부 연결 완료 — **배선 자체는 크래시 없이 정상 동작함을 실제 140장 데이터로 확인**(depth
로드 140/140장 성공, `rectify_images` 36.6초 완주).

**하지만 실제 시각적 결과를 직접 렌더링해서 비교한 결과 — 명백한 화질 역행 발견**: "적용 전"
(flat-homography only)과 "적용 후"(depth correction) mosaic을 나란히 렌더링해 눈으로 직접
비교 — 적용 후 이미지가 발코니만 고쳐진 게 아니라 **하늘/산/벽면 전체에 소금-후추 스펙클
노이즈가 광범위하게 덮임**, 명백한 품질 저하. `deviation_threshold_m`을 0.15m→0.5m로
올려도 스펙클이 크게 줄지 않아 단순 임계값 튜닝 문제가 아님을 확인 — geometric consistency
필터링을 거친 depth map도 여전히 픽셀 단위 노이즈가 상당하고(특히 유리창/반사면 근처),
forward-scatter(점 단위로 목적지에 흩뿌리는 방식) 자체가 이 노이즈에 매우 취약해서, 실제
발코니가 아니라 평범한 벽면의 depth 추정 잡음까지 "off-plane"으로 잘못 분류돼 화면 전체에
번짐. 배경(하늘/산, 30-90m 이격)이 직교투영 특성상 캔버스 안쪽으로 잘못 흘러들어오는 별도
버그도 발견/수정(`max_deviation_m` 상한 추가로 해결 확인됨)했지만, 벽면 자체의 노이즈
문제는 이 세션에서 해결하지 못함.

**조치**: `pipeline.yaml`의 `dense.enable`을 `false`로 되돌려 실제 운영 품질 저하를 방지.
코드/배선은 그대로 남겨둠(향후 재작업 시 재사용 가능) — `dense_depth.py` 상단 주석과
`pipeline.yaml`의 `dense.enable` 주석에 이번 실측 결과와 원인, 재활성화 전 필요한 개선
방향(공간적 일관성 검증 — 이웃 픽셀 다수가 동의해야 진짜 off-plane으로 인정하는 방식, 또는
최소 연결 성분 크기 필터 등, 단일 픽셀 잡음 하나로 판정하지 않도록)을 명시해둠. **결론:
GPU dense reconstruction 빌드 자체(Option D의 인프라)는 완성/검증됐지만, 그걸 실제 발코니
왜곡 수정에 쓰는 방법(현재의 순수 픽셀 단위 forward-scatter)은 이번 실측에서 효과가
없었고 오히려 해로웠음 — "빌드가 됐다"와 "그 결과를 프로덕션에 안전하게 통합했다"는 서로
다른 주장이라는 걸 실측으로 확인한 사례.**

## 2026-09-10: "이중선"/"물결치는 건물" 결함 — 하루 종일 여러 개의 잘못된 진단 끝에 진짜 원인 발견 (align_reconstruction_to_utm 누락)

**배경**: 09-09 밤~09-10 새벽에 걸쳐 COLMAP 정류 결과물의 발코니 코너 "이중선"(ghosting) 결함과,
사용자가 스크린샷으로 지적한 훨씬 큰 결함("물결치듯 휘어진 건물 실루엣")을 조사. 최종 원인은
**둘 다 같은 근본 문제**였음: 그날 만든 스크래치 검증 스크립트들이 실제 프로덕션 코드
(`runner.py`)가 항상 호출하는 `align_reconstruction_to_utm`(GPS 기반 UTM+고도 정렬)을
빠뜨리고 `pycolmap.Reconstruction(sparse_dir)`를 로드한 직후 바로
`facade_plane_from_reconstruction`을 호출했음. 이 미정렬 상태에서는 Z축이 "고도(위)"가
아니라 카메라들의 평균 촬영 방향(이 데이터셋에선 우연히 "벽면에 수직인 깊이 방향")과
일치하는, 완전히 임의의 축이 됨 — `sparse/best`(디스크 상의 COLMAP 원본 출력)는 항상
이 미정렬 상태이고, `align_reconstruction_to_utm`은 메모리 안에서만 변환하고 디스크에
다시 쓰지 않기 때문에, 이 단계를 빠뜨리는 실수는 겉보기엔 "그럴듯한" 결과(위성/GPS 스케일
덕분에 실제 미터 단위처럼 보이는 width_m/height_m)를 내놓으면서도 방향(orientation)은
완전히 틀린, 알아채기 매우 어려운 종류의 버그였음.

**연쇄적으로 나온 잘못된 진단들** (전부 이 미정렬 상태에서 테스트했기 때문에 발생):
1. **"발코니 코너 이중선은 해상도 부족(다운샘플링 앨리어싱) 때문"** — 미정렬 상태에서 계산한
   건물 폭이 21.8m(13층 건물에 물리적으로 불가능한 크기, 층당 1m도 안 됨 — 이 명백한
   모순을 그 자리에서 못 잡았음)로 나와서 "원본 사진 5280px 대비 10배 다운샘플링"이라고
   오판, `px_per_m`을 100→300으로 올려서 "해결"했다고 보고함. 실제 정렬된 건물 폭은
   63.5m이고, 그 기준으로는 px_per_m=100에서 다운샘플링이 2배도 안 됨 —애초에 "해상도
   부족" 진단 자체가 성립하지 않았음.
2. **"발코니 코너 이중선은 seam/블렌딩 알고리즘 문제"** — num_bands 축소, 하드심(blend_analysis)
   전환, 노출보정+하드심 조합까지 시도했으나 전부 그래디언트 프로파일 객관 비교로 실패 확인
   (단, 이 과정에서 "시각적으로 봤을 때 고쳐진 것 같다"는 판단을 그래디언트 수치 비교 없이
   두 번이나 잘못 내렸음 — 매번 나중에 수치로 재검증해서야 오판임을 발견).
3. **"건물이 물결치는 건 COLMAP 번들 조정의 실제 카메라 롤(roll) 추정 노이즈 때문"** — 미정렬
   상태에서 측정한 roll이 이미지마다 최대 12도까지 들쭉날쭉해서, DJI 짐벌 메타데이터
   (`GimbalRollDegree`, IMU 기반 실측값)와 비교해 "COLMAP 추정 오차"라고 결론짓고 그
   격차만큼 카메라를 광축 중심으로 회전시키는 사후보정 코드(`compute_gimbal_roll_corrections`)를
   작성/적용함 — 적용해보니 건물이 오히려 훨씬 심하게 뒤틀림. 원인: 미정렬 상태의
   `plane.e_v`가 실제 수직([0,0,1])과 무관한 축이라(내적 -0.12), "plane.e_v 기준 roll"과
   "진짜 중력 기준 roll"(짐벌 값)을 비교한 것 자체가 서로 무관한 두 기준을 비교한 것이었음.

**진짜 원인 발견 과정**: roll 보정이 오히려 악화시킨 것을 보고 `plane.e_v`와 진짜 세계
수직([0,0,1])의 내적을 직접 찍어봄 → -0.12(거의 무관)로 나와서 이상함을 감지 → 카메라의
forward 방향을 세계 좌표로 변환해보니 거의 정확히 Z축과 일치(0.9995) → 이건 "카메라가
거의 수직 위쪽을 보고 있다"는 뜻인데 벽면 정면 촬영에서 있을 수 없음 → 카메라 위치의 Z값이
전부 0.01~0.02 수준(고도라면 말이 안 되는 값) → `align_reconstruction_to_utm`을 실제로
호출한 적이 없다는 것을 그제서야 확인. 정렬 후 재측정: `plane.e_v` 내적 -1.0(정확히 진짜
수직), 건물 폭 63.5m, DJI_0045의 측정 roll 0.015도(짐벌 값 0.10도와 거의 정확히 일치),
**140장 전체에서 COLMAP-짐벌 roll 차이 평균 -0.08도·표준편차 0.003도**(원래 "12도까지
노이즈"라던 것과 전혀 다름 — roll 추정 자체는 처음부터 매우 정확했음). 정렬 후 px_per_m=100
그대로 재렌더링하니 물결도, 발코니 이중선도 둘 다 사라짐 — 결국 **알고리즘 수정이 전혀
필요 없었고, 제 테스트 절차를 프로덕션과 동일하게(정렬 포함) 맞추는 것만으로 두 결함 모두
해결됨**.

**되돌린 것**: `compute_gimbal_roll_corrections`/`_measure_roll_deg`/`_rotz_deg`/
`_read_gimbal_roll_degree` 전체 삭제(존재하지 않는 문제를 고치던 코드라 재사용 가치
없다고 판단, 완전 제거), `facade_plane_from_reconstruction`/`fit_plane_from_points`의
`px_per_m` 기본값 300 → 100 원복(코드 주석에 이번 조사 결과와 이유 남김).

**교훈**:
- **재구성(reconstruction)을 다루는 스크래치 검증 스크립트는 반드시 실제 파이프라인과
  동일한 전처리(특히 `align_reconstruction_to_utm`)를 거쳐야 함** — 이 단계 없이는
  width_m/height_m 같은 "그럴듯해 보이는" 파생값조차 신뢰할 수 없고, 그 위에 쌓은 모든
  후속 진단(해상도, roll, 등)이 전부 오염됨.
- **"물리적으로 말이 되는가"를 계산값 하나하나에 항상 적용할 것** — 13층 건물이 12.6m라는
  값(층당 1m 미만)은 그 자리에서 바로 이상 신호였는데 놓쳤음. 실측 수치라도 상식적
  타당성 검사를 거르면 안 됨.
- **시각적으로 "고쳐진 것 같다"는 판단은 이번 조사에서 최소 2번 틀렸음** — num_bands 비교,
  그리고 roll 보정 자체("여러 번 렌더링을 비교하며 판단" 단계에서). 매번 나중에
  그래디언트 프로파일 등 객관적 수치 비교로 재검증하고 나서야 오판임을 발견함 — 미세한
  기하학적 결함은 반드시 수치로 비교할 것, 눈으로 두 이미지를 번갈아 보는 것만으로는
  불충분함.
- 사용자가 "시뮬레이션 값이라 그런 것 아니냐"고 두 번 제동을 건 것(GimbalRollDegree=0
  이 시뮬레이션 특성일 수 있다는 지적, 그리고 크기 확대로 인한 착시 가능성 지적)이 매번
  실제로 조사 방향을 바로잡는 데 결정적이었음 — 사용자의 "이게 진짜 맞나?" 질문을
  가볍게 넘기지 말 것.

**다음에 확인할 것 (2026-09-10, 정렬 수정 후 최신 FRONT_visual_colmap.tif에서 사용자가
스크린샷으로 지적, 아직 조사 안 함)**:
1. 왼쪽 벽면에 작은 모서리(돌출부)가 아직 남아있음 — 코너 마스킹
   (`compute_image_valid_x_ranges`)이 못 잡는 더 작은 규모의 잔여 코너 조각으로 추정.
2. 옥상/파라펫 부분 — 원형 환기구 2개 사이의 분홍색 계단실(옥탑) 구조물 주변에서 상단
   연결선(스카이라인)이 이미지 간에 안 맞음.
3. 세 번째 기둥(모서리 필라스터, 원형 환기구 2개가 있는 좁은 세로 기둥)이 이미지 정합
